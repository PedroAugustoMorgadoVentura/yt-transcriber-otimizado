...
def _expand_english_contractions(text: str) -> str:
    """
    Expande contrações do inglês de forma robusta.
    Funciona com apóstrofos curvos/diferentes e ignora capitalização.
    """

    # Normaliza todos os tipos de apóstrofos para um padrão só
    text = re.sub(r"[’‘´`]", "'", text)

    # Dicionário PRO de contrações
    contractions = {
        ...
        "it's": "it is",
        "let's": "let us", "mightn't": "might not", "might've": "might have",
        "mustn't": "must not", "must've": "must have",
        "shan't": "shall not", "she'd": "she would", "she'll": "she will",
        "she's": "she is", "should've": "should have",
        "shouldn't": "should not",
        "that'd": "that would", "that'll": "that will", "that's": "that is",
    ...
    # Baixa TODAS as chaves para lowercase
    contractions = {k.lower(): v for k, v in contractions.items()}

    # Regex de contrações (preserva maiúsculas depois)
    pattern = re.compile(
    ...
    def replace(match):
        ...
        key = original.lower()
        expanded = contractions.get(key, original)

        # Mantém maiúscula inicial se necessário
